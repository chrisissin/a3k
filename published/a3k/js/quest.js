...
var q_title=getQuestTitleFromList(q_id);notifyactionresult('['+q_title+'] 完成 ! ');removeQuestFromList(q_id);clearQuestDetail();Pexpcount(getplayer());loadqestionlist(getplayer());LoadPlayerInfo(getplayer());},'json');});clearQuestButton();$('#questList1').click(getRequester).find('a').addClass('mission_btn');$('#questList2').addClass('mission_btn_selected');$('#questList3').click(getQuestFinishedList).find('a').addClass('mission_btn');$.ajax({type:"POST",dataType:"json",url:"./dispatchers/dispatcher.php",data:{f:'quest',me:getplayer(),act:'getAcceptedList'},success:function(data){handleQuestList(data);if(show_q_id){$('.mission_listtext > div > a[id="'+show_q_id+'"]').click();}},error:function(){refershonerror();},timeout:function(){refershonerror();}});});}
...
removeQuestFromList(q_id);clearQuestDetail();loadqestionlist(getplayer());},'json');});});$.ajax({type:"POST",dataType:"json",url:"./dispatchers/dispatcher.php",data:{f:'quest',me:getplayer(),act:'getQuestList',requester:name},success:function(data){handleQuestList(data);},error:function(){refershonerror();},timeout:function(){refershonerror();}});}
...
for(var i in data.list){var s=data.list[i].split(',');$list.append('<div><a id="'+s[0]+'" >'+s[1]+'</a></div>');if(s[2]&&parseInt(s[2],10)==2){$('#'+s[0]).before('<span>[可完成]</span>');}}
$('.mission_listtext a').click(getQuestDetail);$list.css('height','265px');}
