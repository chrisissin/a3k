...
$('#popup').load('./npclist.html',{},function(){$(this).showPopup();var tool=$('#maindiv');var toolPosition=tool.offset();moveTo($(this),{x:toolPosition.left+100,y:toolPosition.top+100});});$("#popup").draggable();});$('#map_btn_find').click(function()
{if($('#popup').length==0){$('<div></div>').attr('id','popup').css('position','absolute').hide().popup({linkSensor:false,outSensor:false}).appendTo('body');}
$('#popup').load('./look.html',{},function(){$(this).showPopup();var tool=$('#maindiv');var toolPosition=tool.offset();moveTo($(this),{x:toolPosition.left-20,y:toolPosition.top-20});});$("#popup").draggable();});$('#map_btn_rest').click(function()
...
{loadimat(getplayer());}});function knockknock()
...
{$.ajax({type:"POST",dataType:"html",cache:false,url:"./dispatchers/dispatcher.php",data:{me:""+me+"",citi:""+citi+"",what:"moveme2citi"},success:function(data){eval(data);},error:function(){refershonerror();},timeout:20000});}}
function loadimat(player)
{$.ajax({type:"POST",dataType:"json",url:"./dispatchers/dispatcher.php",data:{what:"loadimat",me:""+player+""},success:function(data){showimat(data);},error:function(){refershonerror();},timeout:20000});}
...
{$.ajax({dataType:"json",type:"POST",url:"./dispatchers/dispatcher.php",data:{state:""+state+"",what:"loadciti"},success:function(data){loadingOff();loadcities_back(data)},error:function(){refershonerror();},timeout:20000});}
