...
{loadingOn();if($('#popup').length==0){$('<div></div>').attr('id','popup').css('position','absolute').hide().popup({linkSensor:false,outSensor:false}).appendTo('body');}
...
{$.ajax({type:"POST",url:"./dispatchers/dispatcher.php",data:{me:""+me+"",what:"heal"},success:function(data){loadingOn();notifyactionresult(data);LoadPlayerInfo(getplayer());loadingOff();g_infight=1;},error:function(){refershonerror();},timeout:20000});}
...
{document.getElementById('thWholething').style.backgroundImage='url('+net+'/images_scen/'+document.getElementById('plshit').value+'.jpg)';$.ajax({type:"POST",dataType:"script",url:"./dispatchers/dispatcher.php",data:{what:"fight_init",defencer:danpc,fkind:fkind,me:""+getplayer()+""},success:function(data){bigdick(data);fightdivcontrolOn();},error:function(){refershonerror();},timeout:20000});}
...
{var shwstr=data+$('#executionfightjq').html();eval(data);}}
...
{var shwstr=data.msg2user+"<hr/>"+$('#messagetext').html();notifyactionresult(data.msg2user);}
