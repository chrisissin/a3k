...
{$('#'+data.mytree[i].role_id).css("background-image","url("+net+"/images_ico/"+data.mytree[i].role_img+".gif)");if(data.mytree[i].abc!=undefined)
...
{if(data){$('#spanparent').text(data.role_parentname);$('#spanstrlmt').text(data.role_lmt_str);$('#spanbonelmt').text(data.role_lmt_bone);$('#spanagllmt').text(data.role_lmt_agl);$('#spansmartlmt').text(data.role_lmt_smart);$('#spanvitlmt').text(data.role_lmt_vit);$('#divroledesc').text(data.role_descri);$('#imgroleskill').attr({src:""+net+"/images_ico/"+data.role_skillimg+".gif",title:data.role_skillname,alt:data.role_skillname});}}
function changerole(roleid,me)
{if(roleid!=undefined)
{$.post("./dispatchers/dispatcher.php",{roleid:""+roleid+"",me:""+me+"",what:"changerole"},function(data){exeJSstring(data);LoadPlayerInfo(getplayer());},'text');}}