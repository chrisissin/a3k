...
function mapmovemsg(msg)
{
        ...
        $('#mmppopup').load('./message_map.html', {}, function() {
            $(this).showPopup();
            var tool = $('#maindiv');
            var toolPosition = tool.offset();
...
function heal(me)
{
	toheal(me);
       /*
        $('#popup').load('./healopt.html', {}, function() {
            $(this).showPopup();
          });
          */
}
...
function loadqestionlist(me) {
  if (me){
	    ...
	    url: "./dispatchers/dispatcher.php",
			data: {
me: "" + me + "", what: "loadquestionlist" 
				},
			success: function(data) {
display_question_list(data);
			}, 
			error:
			function() {
			}, 
...
function checkplayer()
{
	if(getplayer() == null)
	{
            heaven("text", enteringgame_res, {what: "checkplayer" } );
            /*
			$.ajax({
				dataType: "text",
		  type: "POST",
...
{debugger;
	alert('對不起 , 突然斷線, 請按F5重新取得連線.建議使用Firefox');
}

function heaven(datatype, callback , updata)
{//debugger;
    	$.ajax({
			dataType: datatype,
		        type: "POST",
