...
function display_more(data){$("#leftbth").unbind('click');$('#leftbtn').bind("click",function(){display_fellow_list(data);});$('#fellowtable').html("");var aline="<table border='0' height='50' cellspacing='0' cellpadding='0' ><tr>";var j=5;if(data.length>5){for(var i=0;i<5;i++){if(j<data.length){var flimg=data[j].p_img;var flname=data[j].p_name;var flid=data[j].p_id;var idstr="'"+flid+"'";var namestr="'"+flname+"'";var face="\
              <td width='50' onclick=\"getfellowinfo("+flid+")\">\
               <a ><ul class='pick'>\
                  <li class='fighticon'>\
...
function equipfellow(flid,me){equipedfellow=flid;if(flid!=undefined){$.post("./dispatchers/dispatcher.php",{flid:""+flid+"",me:""+me+"",what:"equipfellow"},function(data){epupres(data);LoadPlayerInfo(getplayer());},'text');}
else
notifyactionresult("??");}
function epupres(res)
{var output="";if(res)
output="OK";else
output="??";notifyactionresult(output);}