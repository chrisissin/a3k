...
function display_more(data){$("#leftbth").unbind('click');$('#leftbtn').bind("click",function(){display_fellow_list(data);});$('#fellowtable').html("");var aline="<table border='0' height='50' cellspacing='0' cellpadding='0' ><tr>";var j=5;if(data.length>5){for(var i=0;i<5;i++){if(j<data.length){var flimg=data[j].p_img;var flname=data[j].p_name;var flid=data[j].p_id;var idstr="'"+flid+"'";var namestr="'"+flname+"'";var ffaces="url("+net+"/images_char/icon/left/"+flimg+".gif)";var face="\
              <td width='50' onclick=\"getfellowinfo("+flid+")\">\
               <a ><ul class='pick' style='background-image :"+ffaces+";'>\
                  <li class='fighticon'>\
                  </li>\
...
function getfellowinfo(fellowid){$.post("./dispatchers/dispatcher.php",{fellowid:""+fellowid+"",what:"getfellowinfo"},function(data){disp_singlefellow(data);selectedfellow=fellowid;},'json');}
...
{var output="";if(res)
output="OK";else
refershonerror();notifyactionresult(output);}