...
$(document).ready(function() {
    ...
    $('#pllastpg').bind("click", function() {
    	if(plpgfrom-10 > 0)
    	{
    	  plpgfrom = plpgfrom -10;
    	  plpgto = plpgto-10;    		
        getplist(getplayer(), plpgfrom , plpgto,$('#lookfor').val().trim());
      }
    });
    ...
    $("#lookforbtn").unbind('click');
    $('#lookforbtn').bind("click", function() {
    	  plpgfrom = 0;
    	  plpgto = 10;
    	  if($('#lookfor').val().trim() != "")
        	getplist(getplayer(), plpgfrom , plpgto, $('#lookfor').val().trim());
        else 
        	notifyactionresult('請輸入名子');
    });        
    
    
    getplist(getplayer(), plpgfrom , plpgto, "");
    clearinfo()
});

function getplist(me, from , to, sname) 
{
						$.ajax({
					  type: "POST",
					  dataType: "json",
				    ...
				    url: "./dispatchers/dispatcher.php",
						data: {
					 what :'getplist', me:getplayer(),  from: ""+from+"" , to : ""+to+"" , sname: ""+sname+""
							},
...
function getplist_res(data)
{
				...
				if(data != null) {
				  ...
				  {
				  	thehtml = "<a onclick='getinfo("+data[i]["p_id"]+");' >"+data[i]["p_name"] + "</a><br/>";
...
function getinfo_res(data)
{
    ...
    $('#player-loss').text(data.p_lost);
		$('#player-guild').text(data.p_guild);
    ...
    $('#player-guild').text(data.g_name);
    
    $('#player-job').text(data.role_name);


		$("#player-img").css("background-image","url("+net+"/images_char/icon/left/"+data.p_img+".gif)");
