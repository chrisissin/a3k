...
function showFullInfo(data)
{
    ...
    $('#skill_point').text(data.skill_point);
    $('#role_skillpointmax').text(data.role_skillpointmax);
    ...
    $('#player-probability').text(parseInt(count));
		...
		$("#player-img").css("background-image","url("+net+"/images_char/still/left/"+data.p_img+"_2.gif)");
		//$("#player-img").attr('src',"/a3k/images_char/still/left/"+data.p_img+"_2.gif");
		
		$("#fighticon").css("background-image","url("+net+"/images_char/icon/left/"+data.pf_img+".gif)");
