...
$(document).ready(function(){
    ...
    $("#eqptbuy").unbind('click');
    $("#eqptbuy").bind('click',function(){
    		eqptinit();
    	});
    	
    $("#itembuy").unbind('click');
    $("#itembuy").bind('click',function(){
    		iteminit();
    	});
    	
    $("#itemsell").unbind('click');
    $("#itemsell").bind('click',function(){
    		siteminit();
    	});
    
    $("#eqptsell").unbind('click');
    $("#eqptsell").bind('click',function(){
    		seqptinit();
    	}); 
    	
    $("#sellit").unbind('click');
    $('#sellit').bind("click", function() {
        $('#popup').hidePopup();
        loadingOn();
        if ($('#popup').length == 0) {
            $('<div></div>')
    ...
    $("#buybtn").unbind('click');
			$('#buybtn').bind("click",function()
			{
				if (selectedSingleid==getplayer())
					buyitem(selectedSingleid,selectedItemid);
				else
					notifyactionresult("請選擇物品");
			});
    ...
    $("#sellbtn").unbind('click');
			...
			{
				if (selectedSingleid==getplayer())
					sellitem(selectedSingleid,selectedItemid,getplayer());
				else
					notifyactionresult("請選擇物品");
			});
	...
	//@liao---------------function for buy-------------------------------------	
	function eqptinit()
{
      disp_initialitem();
      selectedItemid="";
			getshopInfo(getplayer(), '1');	 		
...
function buyitem(theid, itemid)
{
	if(theid!= undefined){
	    ...
	    url: "./dispatchers/dispatcher.php",
			data: {
theid: ""+theid+"" , itemid: ""+itemid+"" ,   what: "buyitem"
				},
			success: function(data) {
	  	  	 		exeJSstring(data);
	  	  	 		getmoneyinfo(getplayer());
...
function stagdisplay(whichone)
{
	...
	$('#'+whichone+'box').addClass('mission_btn_selected');
	
	$('#eqptbuy').addClass('mission_btn');
	$('#itembuy').addClass('mission_btn');
	$('#'+whichone).removeClass('mission_btn');	
...
function selltagdisplay(whichone)
{
	...
	$('#eqptsell').addClass('mission_btn');
	$('#itemsell').addClass('mission_btn');
	$('#'+whichone).removeClass('mission_btn');	
	...
	function closeThis()
{
	...
	function getshopInfo(me,type1){
					 $.post("./dispatchers/dispatcher.php", {type1: ""+type1+"" , me: ""+me+"" ,  what: "getshopInfo" },
	  	  	 function(data){ display_shop(data); 
	  	  	 	getmoneyinfo(me); },'json'
	  	  				); 	
		}
	
	function getmoneyinfo(me){
						$.post("./dispatchers/dispatcher.php", { me: ""+me+"" ,  what: "getMoneyInfo" },
	  	  	 function(data){ disp_money(data);  },'json'
	  	  				); 
		}
	...
	function display_shop(data)
{
	...
	{		
		for(var i =0; i < data.length; ++i)
		{
		  ...
		  aline+= thecell;             
			...
			{
				celcount = 0;
				$('#shoplisttbl').html($('#shoplisttbl').html()+aline+"</tr>");
				aline = "<tr>";
			}
			++celcount;
		}
    ...
    $('#rmoney').text(data.p_r_money);
}
	
	
	function gettheiteminfo(itemid)
{
		  	  	$.post("./dispatchers/dispatcher.php", {itemid: ""+itemid+"" , what: "gettheiteminfo" },
	  	  	 function(data){  
	  	  	 			disp_singleitem(data);
	  	  				selectedSingleid = getplayer();
...
function disp_singleitem(data) {
	var sellprice=data.item_price/10;
	var checkmod=sellprice%1;
	if (checkmod!=0)
			sellprice+=(1-checkmod);
		$('#item_name').text("");
		$('#item_ef_type1').text("");
		$('#item_ef_type2').text("");
		$('#item_ef_price').text("");
		...
		$('#item_ef_type1').text(data.item_type1);
		$('#item_ef_type2').text(data.item_type2);
		$('#item_ef_price').text(data.item_price);
		$('#item_sell_price').text(sellprice);
...
//@liao----------------end of finctions for buy-----------------

	function seqptinit()
{
      disp_initialitem();
      selectedSingleid="";
			getPackInfo(getplayer(), '1');	 		
			

			selltagdisplay('eqptbtn');
}
...
function siteminit()
{
      ...
      selectedSingleid="";
			...
			selltagdisplay('itembtn');
}


function getPackInfo(me, type1)
{
		  	  	$.post("./dispatchers/dispatcher.php", {type1: ""+type1+"" , me: ""+me+"" ,  what: "getPackInfo" },
	  	  	 function(data){ display_pack(data); getmoneyinfo(me,type1);  },'json'
	  	  				); 
