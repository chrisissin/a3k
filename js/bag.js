...
$("#shortcutbtn").unbind('click');
$('#shortcutbtn').bind("click", function() {
shortcutitem();
});
selectedItemid = "";
equpinit();
...
$('#eqptitembtn').html("裝備");	$('#lossitembtn').html("丟棄");	$("#lossitembtn").show();	tagdisplay('eqptbtn');
    ...
    //call auction_input
    $("#auctionbtn").unbind('click');
    $('#auctionbtn').bind("click", function() { auctionbtn_input(selectedSingleid); });
}
...
function normbtninit() {
    ...
    $('#lossitembtn').bind("click", function() {
        lossitem(selectedSingleid, selectedItemid, getplayer());
    }); getPackInfo(getplayer(), '3'); $("#whichshortcut").hide(); $("#shortcutbtn").hide(); $("#eqptitembtn").hide(); $('#lossitembtn').html("丟棄"); $("#lossitembtn").show(); tagdisplay('normbtn');
}
function spcbtinit() {
    disp_initialitem();
    selectedSingleid = "";
...
function disp_singleitem(data) {
    ...
    document.getElementById('dascitemid').value = data.item_id;
}
...
function usespecialitem(me, theid, itemid) {
    if (theid) {
	    ...
	    url: "./dispatchers/dispatcher.php",
			data: {
me: "" + me + "", theid: "" + theid + "", itemid: "" + itemid + "", what: "usespecialitem" 
				},
			success: function(data) {
				exeJSstring(data);
				getFullInfo(me);
...
function lossitem(theid, itemid, me) {
    if (theid) {
	    ...
	    url: "./dispatchers/dispatcher.php",
			data: {
theid: "" + theid + "", itemid: "" + itemid + "", me: "" + me + "", what: "lossitem"
				},
			success: function(data) {
				exeJSstring(data);
				getFullInfo(me);
...
function shortcutitem() {
    ...
    if (scp == "" || !IsNumeric(scp)) {
	    ...
	    url: "./dispatchers/dispatcher.php",
			data: {
me: "" + me + "", itemid: "" + itemid + "", scp: "" + scp + "", what: "shortcutitem" 
				},
			success: function(data) {
				shortcutresult(data);
			}, 
...
function auctionbtn_input(pack_id) {
    if (selectedSingleid) {
		...
		$('#salepopup').load('./message_auction.html', {}, function() {
			$(this).showPopup();
			var tool = $('#maindiv');
...
{
	$.post("./dispatchers/dispatcher.php",{me:""+getplayer()+"",what:"resetauction"});
}
