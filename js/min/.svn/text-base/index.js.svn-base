...
$(document).ready(function(){$('#teambtn').click(function(){notifyactionresult("未開放");});$('#guildbtn').click(function(){loadingOn();if($('#popup').length==0){$('<div></div>').attr('id','popup').css('position','absolute').hide().popup(false).appendTo('body');}
$('#popup').load('./group.html',{},function(){$(this).showPopup();var tool=$('#maindiv');var toolPosition=tool.offset();moveTo($(this),{x:toolPosition.left-20,y:toolPosition.top-20});loadingOff();});$("#popup").draggable();});$('#eventbtn').click(function(){notifyactionresult("功能尚未開放");});$('#downtownbtn').click(function(){toDownTown(getplayer());});$('#savecash').click(function(){$('#savecash').attr('href',Cashaddress);});$('#storeindex').click(function(){loadingOn();if($('#popup').length==0){$('<div></div>').attr('id','popup').css('position','absolute').hide().popup(false).appendTo('body');}
$('#popup').load('./store.html',{},function(){$(this).showPopup();var tool=$('#maindiv');var toolPosition=tool.offset();moveTo($(this),{x:toolPosition.left-20,y:toolPosition.top-20});loadingOff();});$("#popup").draggable();});$('#moveit').click(function(){$("#maindiv").load("/a3k/map.html");});$('#logoff').click(function(){setplayer(null);seta3klogin();checkplayer();});$("#maindiv").load("/a3k/map.html");loadingOn();$('#statusbtn').click(function(){loadingOn();if($('#popup').length==0){$('<div></div>').attr('id','popup').css('position','absolute').hide().popup({linkSensor:false,outSensor:false}).appendTo('body');}
$('#popup').load('./player_info.html',{},function(){$(this).showPopup();var tool=$('#maindiv');var toolPosition=tool.offset();moveTo($(this),{x:toolPosition.left-20,y:toolPosition.top-20});loadingOff();});$("#popup").draggable();});$('#packbtn').click(function(){loadingOn();if($('#popup').length==0){$('<div></div>').attr('id','popup').css('position','absolute').hide().popup({inkSensor:true,outSensor:true}).appendTo('body');}
...
{for(var i=0;i<9;++i)
{$('#shortcut_img_'+(i+1).toString()).css("background-image","url("+net+"/a3k/images/icon.gif)");}}
...
{$('#shortcut_img_'+data[i].sc_pos).css("background-image","url("+net+"/a3k/images_ico/"+data[i].sc_img+".gif)");}}}}
