...
function showGroupMember(page,name){checkPage('member','./group2.html',_getData);function _getData(){$.ajax({data:{f:'group',act:'getMember',me:getplayer(),page:isNaN(page)?1:page,name:name?name:""},success:function(data){setCurrentButton('btnMember');if(data.in_group==true){showGroupMemberPager(data.count,data.page);$('#textfield').unbind('keypress').keypress(function(e){var key=e.which;if(key==13){$('#button').click();return false;}});$('#button').unbind('click').click(function(){var name=$('#textfield').val();showGroupMember(1,name);});var list=data.list;var $list=$('#member_list > tbody').empty();var i=0,count=0;if(list.name){count=list.name.length;}
for(;i<count;i++){$('<tr><td height="24" align="center" valign="top">'+list.name[i]+'</td><td height="24" align="center" valign="top"></td><td height="24" align="center" valign="top">'+list.role[i]+'</td><td height="24" align="center" valign="top">'+list.lv[i]+'</td><td height="24" align="center" valign="top"></td></tr>').appendTo($list);}
for(;i<10;i++){$('<tr><td>&nbsp;</td><td></td><td></td><td></td><td></td></tr>').appendTo($list);}
