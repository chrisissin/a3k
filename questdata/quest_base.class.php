...
class quest_base{
	...
	function checkLimit(&$p) {
		...
		if($this->limit['item']) {
		 ...
		 if($this->limit['quest']) {
		 	...
		 	$check = $db->query_first('SELECT count(q_id) AS num FROM a3k_quest_status WHERE q_id in ('.$quest.') AND q_status = -1 AND p_id = '.$p['p_id'].' ');
	...
	//	echo("<script>notifyactionresult($query);</script>");
